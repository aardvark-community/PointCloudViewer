...
type PointCloudInfo =
    {
        ...
        normalVisSurface : Option<FShade.Effect>
...
module PointCloudRenderObjectSemantics = 
    ...
    module PointCloudRenderer = 
        ...
        type private LoadedGeometry(pool : IGeometryPool, ptr : Management.Block<unit>, geometry : Option<IndexedGeometry>, plane : Option<V3f[]>) =
            let range =
                match geometry with
                    | Some g -> Range1i(int ptr.Offset, int ptr.Offset + int ptr.Size - 1)
                    | None -> Range1i.Invalid

            static let empty = LoadedGeometry(Unchecked.defaultof<_>, Unchecked.defaultof<_>, None, None)
...
//                    Log.stop()
                ...
                let load (ct : CancellationToken) (node : ILodDataNode) =
                    ...
                    match geometry with
                        | Some g ->
                            ...
                            LoadedGeometry(pool, ptr, geometry, plane)
            ...
            let vertices = 
                let reader = loadedGeometries.GetReader()
                let arrayBuffer = 
                    Mod.custom (fun token -> 
                        reader.GetOperations() |> ignore
                        let geos = reader.State 
                        let vertices =
                            geos 
                            |> Seq.collect (fun o -> 
                                match o.Plane with
                                    | Some ps -> ps
                                    | None -> Array.create 4 V3f.Zero 
                               )
                            |> Seq.toArray
                        ArrayBuffer(vertices) :> IBuffer
                    )
                BufferView(arrayBuffer, typeof<V3f>)

            let plane3d =
                let ro = RenderObject.Clone ro
                match config.normalVisSurface with
                    | None -> RenderObject.Empty
                    | Some s -> 
                        ro.VertexAttributes <- AttributeProvider.ofSeq [ DefaultSemantic.Positions, vertices ]
                        ro.Surface <- Surface.FShadeSimple s
                        ro.Mode <- IndexedGeometryMode.PointList
                        ro.Activate <- fun () -> { new IDisposable with member x.Dispose () = ()}
                        ro
            ...
            ro :> IRenderObject, ro :> IRenderObject

        [<Semantic>]
        type MyPCSem() =
            member x.RenderObjects(m : Sg.PointCloud) =
                let scope = Ag.getContext()
                let obj,plane = createRenderObject scope m.Config m.Data
                ASet.ofSeq [obj;plane]
